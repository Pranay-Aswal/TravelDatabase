...
INNER JOIN customer_details ON bookings.user_uniqueID=customer_details.unique_id)  where isValidated=:isv and trip_details.Date >= CURDATE() and email_id=:eid";//and trip_details.Start_time > CURTIME()
$stmt = $pdo->prepare($sql);
$stmt->execute(array(':isv'=>$_POST['sel'],':eid'=>$_SESSION['email_id']));
...
else if(isset($_POST['all']))
{
    $sql="SELECT * FROM ((`bookings`
INNER JOIN trip_details  ON bookings.unique_tripID=trip_details.Unique_tripID)
INNER JOIN customer_details ON bookings.user_uniqueID=customer_details.unique_id)  where trip_details.Date >= CURDATE() and email_id=:eid";
